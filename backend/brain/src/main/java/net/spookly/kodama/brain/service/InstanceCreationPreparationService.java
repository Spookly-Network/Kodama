...
public class InstanceCreationPreparationService {
  ...
  public PreparedCreateInstanceRequest prepareForCreate(CreateInstanceRequest request) {
    ...
    List<AssignmentDescriptor> assignmentDescriptors =
        resolveAssignmentDescriptors(request, blueprint);
  ...
  public List<InstanceTemplateAssignment> buildAssignments(
      Instance instance, PreparedCreateInstanceRequest preparedCreateRequest) {
            ...
            descriptor ->
                new InstanceTemplateAssignment(
                    ...
                    preparedCreateRequest
                        .assignmentLookup()
                        .templatesById()
                        .get(descriptor.templateId()),
                    descriptor.templateVersionId() == null
                        ? null
                        : preparedCreateRequest
                            .assignmentLookup()
                            .versionsById()
                            .get(descriptor.templateVersionId()),
                    descriptor.priority()))
        .toList();
  }
  ...
  public List<InstanceGroupMembership> buildGroupMemberships(
    ...
    return groups.stream().map(group -> new InstanceGroupMembership(instance, group)).toList();
  }
  ...
  private List<AssignmentDescriptor> resolveAssignmentDescriptors(
      CreateInstanceRequest request, Blueprint blueprint) {
  ...
  private List<AssignmentDescriptor> validateAndNormalizeTemplateAssignments(
      List<TemplateAssignmentRequest> assignments, boolean required) {
    ...
    for (int i = 0; i < assignments.size(); i++) {
      ...
      if (assignment == null) {
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "Template assignment entry is required");
      }
      if (assignment.getTemplateId() == null) {
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "templateId is required for each assignment");
      }
      ...
      descriptors.add(
          new AssignmentDescriptor(
              assignment.getTemplateId(), assignment.getTemplateVersionId(), priority));
    }
    return descriptors;
  }
  ...
  private String resolveVariablesJson(CreateInstanceRequest request, Blueprint blueprint) {
    ...
    if (variables != null && variablesJson != null) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Provide either variables or variablesJson, not both");
    }
    if (variables != null) {
      try {
        ...
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "Unable to serialize variables", e);
      }
    }
  ...
  private String resolveStartCommandJson(List<String> startCommand, Blueprint blueprint) {
    ...
    for (String token : startCommand) {
      if (token == null || token.isBlank()) {
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "startCommand entries must not be blank");
      }
    }
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Unable to serialize startCommand", ex);
    }
  }
  ...
  private String resolvePortDefinitionsJson(CreateInstanceRequest request, Blueprint blueprint) {
    if (request.getPortDefinitions() != null) {
      List<PortDefinitionJson> overrideDefinitions =
          normalizePortDefinitions(request.getPortDefinitions());
    ...
    List<PortDefinitionJson> blueprintDefinitions =
  ...
  private List<PortDefinitionJson> normalizePortDefinitions(
      List<PortDefinitionRequest> definitions) {
    List<PortDefinitionJson> normalized = new ArrayList<>();
    for (PortDefinitionRequest definition : definitions) {
      if (definition == null) {
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "Port definition entry is required");
      }
      ...
      if (containerPort == null || containerPort < 1 || containerPort > 65535) {
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "containerPort must be between 1 and 65535");
      }
      ...
      if (min == null || max == null || step == null) {
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "hostRange min/max/step are required");
      }
      if (min < 1 || min > 65535 || max < 1 || max > 65535) {
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "hostRange min/max must be between 1 and 65535");
      }
      if (step < 1) {
  ...
  private String serializePortDefinitions(List<PortDefinitionJson> definitions) {
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Unable to serialize portDefinitions", ex);
    }
  }

  private List<InstanceGroup> resolveEffectiveGroups(
      CreateInstanceRequest request, Blueprint blueprint) {
  ...
  private AssignmentLookup resolveAssignmentReferences(List<AssignmentDescriptor> descriptors) {
    Set<UUID> templateIds =
        descriptors.stream().map(AssignmentDescriptor::templateId).collect(Collectors.toSet());
    ...
    templateService.ensureTemplatesHaveVersions(
        descriptors.stream()
  ...
  record AssignmentLookup(
      Map<UUID, Template> templatesById, Map<UUID, TemplateVersion> versionsById) {}

  private record PortDefinitionJson(
      String name, String protocol, int containerPort, PortRangeJson hostRange) {}

  private record PortRangeJson(int min, int max, int step) {}
}

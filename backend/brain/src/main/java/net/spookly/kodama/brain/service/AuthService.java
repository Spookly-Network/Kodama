  ...
  public LoginResponse login(LoginRequest request) {
    if (!securityProperties.isEnabled()) {
      throw new ResponseStatusException(
          HttpStatus.SERVICE_UNAVAILABLE, "Authentication is disabled");
    }
    ConfiguredUserStore.StoredUser storedUser =
        userStore
            .findForAuthentication(request.getUsername())
            .orElseThrow(
    ...
    return new LoginResponse(
        token.token(), "Bearer", token.expiresAt(), storedUser.principal().getRoles());
  }
}

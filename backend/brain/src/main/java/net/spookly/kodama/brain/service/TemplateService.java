...
public class TemplateService {
    ...
    public Map<UUID, Template> loadTemplatesById(Collection<UUID> templateIds) {
        ...
        Map<UUID, Template> templatesById = templateRepository.findAllById(deduplicatedIds).stream()
    ...
    public Map<UUID, TemplateVersion> loadTemplateVersionsById(Collection<UUID> templateVersionIds) {
        ...
        Map<UUID, TemplateVersion> versionsById = templateVersionRepository.findAllById(deduplicatedIds).stream()
    ...
    @Transactional(readOnly = true)
    public Map<UUID, TemplateVersion> loadLatestTemplateVersionsByTemplateIds(Collection<UUID> templateIds) {
        if (templateIds == null || templateIds.isEmpty()) {
            return Map.of();
        }
        ...
        Map<UUID, TemplateVersion> latestVersions = selectLatestVersions(
                templateVersionRepository.findLatestForTemplateIds(deduplicatedIds)
        );

...
public class InstanceStateMachine {
  ...
  private Map<InstanceState, Set<InstanceState>> buildAllowedTransitions() {
    ...
    transitions.put(
        InstanceState.STOPPING, EnumSet.of(InstanceState.DESTROYED, InstanceState.STOPPED));
    transitions.put(
        InstanceState.STOPPED, EnumSet.of(InstanceState.DESTROYED, InstanceState.STARTING));
    transitions.put(InstanceState.FAILED, EnumSet.of(InstanceState.DESTROYED));
    ...
    for (InstanceState state : InstanceState.values()) {
      ...
      transitions
          .computeIfAbsent(state, key -> EnumSet.noneOf(InstanceState.class))
  ...
  private void validateTransition(InstanceState currentState, InstanceState targetState) {
    ...
    if (currentState == targetState) {
      throw new InvalidInstanceStateTransitionException(
          "Instance is already in state " + currentState);
    }

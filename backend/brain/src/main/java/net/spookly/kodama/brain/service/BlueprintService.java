...
public class BlueprintService {
  ...
  private static final String BLUEPRINT_DUPLICATE_NAME_MESSAGE =
  ...
  public Blueprint loadBlueprintForInstanceCreation(@NonNull UUID id) {
    ...
    if (blueprint.getDeletedAt() != null) {
      throw new ResponseStatusException(
          HttpStatus.CONFLICT, "Blueprint is deleted and cannot be used for new instance creation");
    }
    return blueprint;
  }
        ...
        .ifPresent(
            existing -> {
              if (currentBlueprintId == null || !existing.getId().equals(currentBlueprintId)) {
                throw new ResponseStatusException(
                    HttpStatus.CONFLICT, BLUEPRINT_DUPLICATE_NAME_MESSAGE);
              }
            });
  }
  ...
  private BlueprintDto toDto(Blueprint blueprint) {
    ...
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.INTERNAL_SERVER_ERROR, "Stored blueprint startCommandJson is invalid", ex);
    }

    if (startCommand == null || startCommand.isEmpty()) {
      throw new ResponseStatusException(
          HttpStatus.INTERNAL_SERVER_ERROR, "Stored blueprint startCommandJson is invalid");
    }

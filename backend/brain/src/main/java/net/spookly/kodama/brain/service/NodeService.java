...
public class NodeService {
  ...
  public List<NodeDto> listNodes() {
    return nodeRepository.findAll().stream().map(NodeDto::fromEntity).toList();
  }
  ...
  public NodeRegistrationResponse registerNode(NodeRegistrationRequest request) {
    ...
    return new NodeRegistrationResponse(node.getId(), nodeProperties.getHeartbeatIntervalSeconds());
  }
  ...
  public NodeDto heartbeat(UUID nodeId, NodeHeartbeatRequest request) {
    ...
    node.updateHeartbeat(
        request.getStatus(), request.getUsedSlots(), OffsetDateTime.now(ZoneOffset.UTC));
  ...
  private Node refreshRegistration(
      Node existing, NodeRegistrationRequest request, OffsetDateTime now) {
  ...
  private void validateHeartbeat(Node node, NodeHeartbeatRequest request) {
    if (request.getUsedSlots() < 0) {
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "usedSlots cannot be negative");
    }

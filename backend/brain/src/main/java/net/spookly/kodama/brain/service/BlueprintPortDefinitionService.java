...
public class BlueprintPortDefinitionService {
  ...
  private static final String DUPLICATE_PORT_NAME_CONSTRAINT =
      "uq_blueprint_port_definitions_blueprint_name";
  private static final String DUPLICATE_PORT_NAME_MESSAGE =
  ...
  public BlueprintPortDefinitionDto addPortDefinition(
      UUID blueprintId, BlueprintPortDefinitionRequest request) {
    ...
    if (hostRangeMin > hostRangeMax) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "hostRange.min must be <= hostRange.max");
    }

    BlueprintPortDefinition portDefinition =
        new BlueprintPortDefinition(
            blueprint, name, protocol, containerPort, hostRangeMin, hostRangeMax, hostRangeStep);
  ...
  public void removePortDefinition(UUID blueprintId, UUID portId) {
    loadBlueprint(blueprintId);
    BlueprintPortDefinition portDefinition =
        blueprintPortDefinitionRepository
            .findById(portId)
            .orElseThrow(
                () ->
                    new ResponseStatusException(
                        HttpStatus.NOT_FOUND, PORT_DEFINITION_NOT_FOUND_MESSAGE));
  ...
  private BlueprintPortDefinitionRequest requireRequest(BlueprintPortDefinitionRequest request) {
    if (request == null) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Port definition request is required");
    }
    return request;
  }

  private BlueprintPortDefinitionRequest.HostRangeRequest requireHostRange(
      BlueprintPortDefinitionRequest request) {
  ...
  private int requirePort(Integer value, String fieldName) {
    ...
    if (value < MIN_PORT || value > MAX_PORT) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, fieldName + " must be between " + MIN_PORT + " and " + MAX_PORT);
    }
    return value;
  }

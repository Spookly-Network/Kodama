...
class BlueprintGroupLinkServiceWriteConflictTest {
    ...
    @BeforeEach
    void setUp() {
        blueprintGroupLinkService = new BlueprintGroupLinkService(
                blueprintRepository,
                instanceGroupRepository,
                blueprintGroupLinkRepository
        );
    }
    ...
    void addGroupLinkTreatsDuplicateKeyViolationAsIdempotentSuccess() {
        ...
        when(blueprintRepository.findByIdAndDeletedAtIsNull(blueprintId)).thenReturn(Optional.of(blueprint));
    ...
    void addGroupLinkPropagatesNonDuplicateIntegrityViolation() {
        ...
        when(blueprintGroupLinkRepository.existsById(new BlueprintGroupLinkId(blueprintId, groupId)))
                .thenReturn(false);
        when(blueprintGroupLinkRepository.saveAndFlush(any(BlueprintGroupLink.class))).thenThrow(failure);

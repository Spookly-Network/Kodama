...
public class BrainAuthFilter extends OncePerRequestFilter {
  ...
  private static final String LEGACY_CERTIFICATE_ATTRIBUTE =
  ...
  private void handleCertificateAuth(
    ...
    if (expectedCertificate == null) {
      response.sendError(
          ...
          "Brain authentication certificate is not configured");
      return;
    }
  ...
  private void handleTokenAuth(
    ...
    if (expectedToken == null || expectedToken.isBlank()) {
      response.sendError(
          HttpStatus.INTERNAL_SERVER_ERROR.value(), "Brain authentication token is not configured");
      return;
    }
    NodeConfig.Auth auth = config.getAuth();
    ...
    if (headerName == null || headerName.isBlank()) {
      response.sendError(
          HttpStatus.INTERNAL_SERVER_ERROR.value(),
          "Brain authentication header is not configured");
      return;
    }

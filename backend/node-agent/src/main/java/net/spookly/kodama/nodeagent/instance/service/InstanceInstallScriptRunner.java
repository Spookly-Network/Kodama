...
public class InstanceInstallScriptRunner {
    ...
    public InstanceInstallScriptRunner(InstanceProperties instanceProperties) {
        this.installScriptTimeoutSeconds = instanceProperties.getInstanceRuntime().getInstallScriptTimeoutSeconds();
    }

    public int runScript(Path workingDir, String script, Map<String, String> env) throws IOException, InterruptedException {
        if (!Files.isExecutable(POSIX_SHELL)) {
            throw new IOException("Install script execution requires /bin/sh, but it is not available on this node");
        }
        ProcessBuilder processBuilder = new ProcessBuilder(POSIX_SHELL.toString(), "-c", script)
        ...
        throw new IOException("Install script timed out after " + installScriptTimeoutSeconds + " seconds");
    }

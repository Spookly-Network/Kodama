...
public class S3TemplateStorageClient implements TemplateStorageClient {
  ...
  public TemplateTarball getTemplateTarball(String templateId, String version, String s3Key) {
    ...
    try {
      GetObjectArgs request = GetObjectArgs.builder().bucket(bucket).object(s3Key).build();
      ...
      if (isNotFound(ex)) {
        throw new TemplateStorageObjectNotFoundException(
            "Template tarball not found in S3 bucket " + bucket + " with key " + s3Key, ex);
      }
      throw new TemplateStorageException(
          "Failed to fetch template tarball from S3 bucket " + bucket + " with key " + s3Key, ex);
    } catch (Exception ex) {
      if (isTimeout(ex)) {

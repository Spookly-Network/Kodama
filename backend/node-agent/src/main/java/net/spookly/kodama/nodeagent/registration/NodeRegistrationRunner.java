    ...
    try {
      ...
      if ((token != null && !token.isBlank()) && (headerName == null || headerName.isBlank())) {
        throw new NodeRegistrationException(
            "Node auth header name is required when a token is provided");
      }
      NodeRegistrationResponse response =
          registrationClient.register(endpoint, headerName, token, request);

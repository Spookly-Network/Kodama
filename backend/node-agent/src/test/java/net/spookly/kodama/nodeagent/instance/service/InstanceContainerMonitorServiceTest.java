...
class InstanceContainerMonitorServiceTest {
  ...
  void monitorRecordsExitCodeWhenContainerStops() {
    ...
    InstanceContainerMonitorService service =
        new InstanceContainerMonitorService(dockerService, registryService, workspaceLayout);
  ...
  void monitorDoesNotOverwriteExitMetadataWhenContainerMissing() {
    ...
    verify(registryService, never()).recordContainerStatus(any(), any(), any(), any(), any());
  }

  private DockerContainerStatus status(
      String containerId, Boolean running, String state, Integer exitCode, String error) {

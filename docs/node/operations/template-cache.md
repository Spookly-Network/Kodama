...
- Added cache population that downloads template archives (tar/tar.gz/zip) from S3, extracts them into a temp directory, and moves them into place atomically.
...
- Use `TemplateCachePopulateService.ensureCachedTemplate(templateId, version, checksum, s3Key)` to download and extract a template archive when cache is missing or invalid.
  - When dev-mode is enabled, the cache lookup is bypassed and the archive is always re-downloaded from S3 (and still written to cache).
  - Downloads the archive to a temp file, extracts into a temp directory, then atomically moves into `<templateId>/<version>`.
  - Writes `checksum.sha256` and `metadata.json` before the atomic move.
  - Validates the downloaded archive checksum (SHA-256) against the expected checksum before writing cache markers.
  ...
  - Restores POSIX permissions from tar/zip entry modes when supported; non-POSIX filesystems fall back to best-effort executable bits.
...
- Archives that exceed extraction limits are rejected and cleaned up.
